...
class ExpenseView(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        return Response(
            {"data": serializer.data, "count": count}, status=status.HTTP_201_CREATED
        )
    ...
    def bulk_delete(self, request):
        ...
        if not ids:
            return Response(
                {"error": "Failed to delete the expense"},
        ...
        return Response(
            {
                "message": f"Deleted {delete_count} data successfully",
            },
...
class RetrieveUserExpense(generics.ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        ...
        response = requests.post(
            fast_api_url + "expense/expense_insight", json=serializer.data
        )
        data = response.json()
        return Response(data, status=status.HTTP_200_OK)
...
def get_expense_trend(request):
    ...
    expenses = [
        {
            ...
            "created_at": str(
                start_date + datetime.timedelta(days=random.randint(1, 14))
            ),
        }
        for i in range(100)
    ]
    try:
        response = requests.post(
            fast_api_url + "expense/trend_analyse",
            json={"expense": expenses, "start_date": start_date.isoformat()},
        )
...
def get_expense_anomaly(request):
        ...
        expense_data.append(
            {
                ...
                "created_at": datetime.fromisoformat(exp["created_at"].replace("Z", ""))
                .date()
                .isoformat(),
            }
        )
    try:
        res = requests.post(
            fast_api_url + "expense/anomaly_detection", json=expense_data
        )
...
def get_total_spend_insight(request):
    ...
    response = requests.get(
        fast_api_url + f"expense/analytics/total_spent",
    ...
    if response.status_code == 200:
        ...
        return Response(
            {"message": "Error while getting the analyzed data"},
            status=status.HTTP_400_BAD_REQUEST,
        )
...
def analyse_reimbrushed(request):
    ...
    response = requests.get(
        fast_api_url + "expense/reimbrush/analyse",
        params={
            "user_id": user_id,
            "month": month,
            "year": year,
...
def analyse_pending(request):
    ...
    # Call the fastapi end point for analytics
    response = requests.get(
        fast_api_url + "expense/pending/analyse",
        params={
            "user_id": user_id,
            "month": month,
            "year": year,
...
def analyse_decline(request):
    ...
    # Call the fastapi end point for analytics
    response = requests.get(
        fast_api_url + "expense/decline/analyse",
        params={
            "user_id": user_id,
            "month": month,
            "year": year,
...
def get_spend_by_categories(request):
    ...
    final_expense = [
        {**expense, "total": round(expense["total"], 2)} for expense in final_expense
    ]
    return Response(
        {
