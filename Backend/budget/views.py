...
class BudgetView(ModelViewSet):
    ...
    def budget_insight(self,request):
        ...
        budgets=self.get_queryset().filter(budget_limit=limit).annotate(
    ...
    def list(self, request, *args, **kwargs):
        ...
        return Response(
            self.get_serializer(budgets,many=True).data,
            status=status.HTTP_200_OK
        )
    ...
    def update_budget_for_reallocation(self,request,pk=None):
        ...
        if int(reallocated_amount)>0 and reallocated_budget_id:
            ...
            if self.get_serializer(source).data['remaining']>=float(reallocated_amount):
    ...
    def get_budget_health(self,request):
        budgets=self.get_queryset().filter(budget_limit=request.GET.get("limit")).annotate(
            total_expense=Coalesce(Sum("budget_expenses__expense_amount"),0,output_field=FloatField())
        )
        ...
        usage_percentage=round((total_spent/total_amount)*100,2) if total_amount>0 else 0
        ...
        return Response(
            {
                ...
                "message":f"{over_budget} categories exceeded.Review needed" if over_budget>0 else "All good",
    ...
    def monthly_spend_analytics(self,request):
        all_expenses=list(Expense.objects.filter(user=request.user).values("expense_amount","budget_id","created_at"))
        formatted_values=[
            {
        ...
        try:
            response=requests.post(settings.FAST_API+"budget/budgetSpend",json=formatted_values)
    ...
    def get_top_spend(self,request):
        # Get all the expense of the user
            budgets=list(self.get_queryset().filter(is_active=True).annotate(
                total_expense=Coalesce(Sum("budget_expenses__expense_amount"),0,output_field=FloatField())
            ))
            formatted=[
                {
                    ...
                    "usage_percentage":round(budget.total_expense*100/budget.budget_amount,2)
                }for budget in budgets
            ]
            return Response(
                formatted,
    ...
    def get_budget_categories_and_type(self,request):
        categories=[
            {"code":code,
             "label":label} for code,label in category
        ]
        types=[
            {
                "code":code,
                "label":lable
            } for code,lable in budget_type_cat
        ]
        return Response({
