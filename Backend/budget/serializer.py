...
class BudgetSerializer(serializers.ModelSerializer):
    ...
    def get_total_spent(self, obj):
        return obj.budget_expenses.aggregate(total=Sum("expense_amount"))["total"] or 0

    def get_remaining(self, obj):
        total_spent = self.get_total_spent(obj)
        return (
            0
            if obj.budget_amount - total_spent < 0
            else obj.budget_amount - total_spent
        )

    def get_usage_percentage(self, obj):
        total_spent = self.get_total_spent(obj)
        return round(
            (total_spent / obj.budget_amount if obj.budget_amount > 0 else 0) * 100, 2
        )
