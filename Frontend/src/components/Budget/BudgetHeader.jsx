import { Download, Grid2X2, List, Table, Target } from 'lucide-react'
...
const BudgetHeader = () => {
    ...
    // Icons name according to the category
    ...
    return (
        <div className='w-full flex justify-between '>
            <div className='flex gap-2 '>
                <div className='size-12 rounded-lg bg-indigo-500 flex items-center justify-center '>
                    <svg
                        width={30}
                        height={30}
                        viewBox="0 0 512 512"
                ...
                <div className='flex  flex-col' >
                    ...
                    <div className='flex gap-1 text-xs text-gray-400 font-semibold'>
                        <span>{new Date().toLocaleDateString("en-US", { month: 'long', year: 'numeric' })}</span>
                        <span>●</span>
                        <span>{budgets.length > 0 ? budgets.length : 0} active budgets</span>
                    </div>
                </div>
            </div>
            {/* Right section */}
            <div className='flex gap-2'>
                ...
                <div className='flex gap-1.5   p-1.5 border bg-white font-semibold border-gray-100 shadow-sm rounded-xl text-[10px]'>
                    {
                        budgetType.map((i, idx) => {
                            const isActive = activeBudgetType === i
                            return (
                                <div key={idx} onClick={() => dispatch(setActiveBudgetType(i))} className={`p-2 rounded-lg transition duration-200  text-center cursor-pointer ${isActive ? "bg-indigo-600  font-bold text-white" : "text-gray-400"}`}>
                                    {i}
                                </div>
                            )
                        })
                ...
                {/* Grid buttons */}
                <div className='flex gap-0.5  p-2  border bg-white font-semibold border-gray-100 shadow-sm rounded-xl text-[10px]'>
                    {
                        views.map((i, idx) => {
                            const isActive = activeView === i.view
                            return (
                                <div key={idx} onClick={() => dispatch(setActiveView(i.view))} className={`rounded-lg p-2 transition duration-200  text-center cursor-pointer ${isActive ? "bg-gray-100  font-bold text-gray-500" : "text-gray-400"}`}>
                ...
                rounded-lg text-white px-4 p-2 cursor-pointer text-sm  transition-all active:scale-95
                '
                >
                    Add Budget
                </button>
