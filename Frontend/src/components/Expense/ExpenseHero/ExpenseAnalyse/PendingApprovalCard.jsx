...
const PendingApprovalCard = () => {
    ...
    useEffect(() => {
        (
            () => {
                setApprovalExpenses(expenses?.filter((item) => item.status === "PENDING"))
    ...
    const handleClick = async () => {
        ...
        try {
            ...
            dispatch(setFilteredExpenses(filteredExpenses?.map((i) => i.status === 'PENDING' ? { ...i, status: 'REIMBRUSHED' } : i)))
    ...
    return (
        <div className='w-full px-2 py-3.5 border border-gray-100 rounded-lg shadow bg-white'>
            <div className='flex flex-col gap-2 text-sm'>
                <span className='text-gray-400 tracking-wider text-[12px] font-semibold pl-2'>NEEDS APPROVAL</span>
...

                    {
                        approvalExpense?.length > 0 ? (
                            approvalExpense.map((items) => {
                                ...
                                return (
                                    <div key={items.id} className='p-2.5 border border-gray-100 bg-slate-50 rounded-xl
                                   flex gap-1.5 justify-between items-center overflow-hidden'>
                                        <div className='flex gap-1'>
                                            ...
                                            <div className='flex flex-col gap-0.5'>
                                                ...
                                                <span className='text-[12px] font-medium text-gray-400'>Rs. {items.expense_amount}</span>
                                            </div>
                                        </div>
                ...
                {
                    approvalExpense.length > 0 && (
                        <button onClick={() => handleClick()} className=' rounded-xl inline-flex gap-2 cursor-pointer  items-center bg-gradient-to-r transition-all duration-200 ease-in active:scale-95  group from-indigo-600 to-indigo-500 justify-center w-11/12 mx-auto py-2 text-white font-semibold text-[14px]'>
